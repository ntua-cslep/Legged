...
// TODO search for spyros' addition to see changes regarding the transsmission of velocity
#include "main.h"
...
int main(void)
 {
   ...
   uint8_t firstIMU = 0;
    ...
    int32_t encoderVel = 0; // spyros' addition
    int32_t dir ; //spyros' addition
    ...
    // TODO: Enable pins for inhibit signal
    ...
    // QEI Setup
        ...
        // TODO
        QEIConfigure(QEI0_BASE, (QEI_CONFIG_CAPTURE_A_B | QEI_CONFIG_NO_RESET | QEI_CONFIG_QUADRATURE | QEI_CONFIG_NO_SWAP), 52*2000-1);
        //
        QEIVelocityConfigure(QEI0_BASE, QEI_VELDIV_64, 4000000);
    ...
    TimerLoadSet(TIMER0_BASE, TIMER_A, g_ui32SysClock/15625);
    ...
    while(1)
    {
      ...
      {
        //sends++;
        encoderPos = (QEIPositionGet(QEI0_BASE)*360/(52*2000-1));
        dir = QEIDirectionGet(QEI0_BASE) ; // spyros' addition
        if (dir == 1)
        {
            encoderVel = QEIVelocityGet(QEI0_BASE) ; // spyros' addition
        }
        else
        {
            encoderVel = - QEIVelocityGet(QEI0_BASE) ; // spyros' addition
        ...
        memcpy(&sendUDP[5],(int8_t*)(&encoderVel),4); // spyros' addition
        udp_send_data((void*)sendUDP,9);
#endif
      }
#endif
    }
